...
def crop_center_square(img):
    """Crops the image to a centered 1:1 square ratio."""
    h, w = img.shape[:2]
    min_dim = min(h, w)
    start_x = (w - min_dim) // 2
    start_y = (h - min_dim) // 2
...
def image_to_ascii_svg(image_path, output_svg_path, width=70):
    ...
    # Crop to square headshot
    cropped_img = crop_center_square(img)

    # Calculate height for proper font aspect ratio (mono font height/width ~ 1.8)
    height = int(width * 0.55)
    resized = cv2.resize(cropped_img, (width, height))
